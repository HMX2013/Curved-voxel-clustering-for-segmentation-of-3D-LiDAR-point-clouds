...
#include <cvc_segment_origin/cvc_segment_origin_Config.h>
...
class cloud_segmentation
{
 private:
  ...
  dynamic_reconfigure::Server<cvc_segment_origin::cvc_segment_origin_Config> server;
  dynamic_reconfigure::Server<cvc_segment_origin::cvc_segment_origin_Config>::CallbackType f;

  pcl::PointCloud<pcl::PointXYZI>::Ptr segmentedCloudColor;

  ros::Subscriber _sub_lidar_points;
  ...
  pcl::PointCloud<PointType>::Ptr roi_filter_pcl(const sensor_msgs::PointCloud2::ConstPtr& laserRosCloudMsg, const float filter_res, const float detection_min, const float detection_max);
  ...
  void CVC_CLUSTER(const pcl::PointCloud<pcl::PointXYZ>::Ptr in_cloud_ptr, autoware_msgs::CloudClusterArray &in_out_clusters, const float deltaA, const float deltaR, const float deltaP);
...
void dynamicParamCallback(cvc_segment_origin::cvc_segment_origin_Config& config, uint32_t level)
{
  // Pointcloud Filtering Parameters
...
cloud_segmentation::cloud_segmentation(){
  ...
  _sub_lidar_points = nh.subscribe(lidar_points_topic_, 1, &cloud_segmentation::lidarPointsCallback, this);
...
pcl::PointCloud<PointType>::Ptr cloud_segmentation::roi_filter_pcl(const sensor_msgs::PointCloud2::ConstPtr& laserRosCloudMsg, const float filter_res, const float detection_min, const float detection_max)
{
  ...
  if (filter_res > 0)
  {
    ...
    return roi_cloud_origin;

}


void cloud_segmentation::CVC_CLUSTER(const pcl::PointCloud<pcl::PointXYZ>::Ptr in_cloud_ptr, autoware_msgs::CloudClusterArray &in_out_clusters, 
...
{ 
  ...
  {
    std::vector<int> in_cluster_indices;
    for (int i = 0; i < cluster_indices.size(); ++i)
    {
      if (cluster_indices[i] == cluster_id[j])
      {
        cluster_color.x = in_cloud_ptr->points[i].x;
        cluster_color.y = in_cloud_ptr->points[i].y;
        cluster_color.z = in_cloud_ptr->points[i].z;
    ...
    cluster->SetCloud(in_cloud_ptr, in_cluster_indices, _velodyne_header, k, 1, 1, 1, "", _pose_estimation);
...
void cloud_segmentation::lidarPointsCallback(const sensor_msgs::PointCloud2::ConstPtr& lidar_points)
{
  ...
  // ROI and filter
  pcl_roi= roi_filter_pcl(lidar_points, VOXEL_GRID_SIZE, DETECT_MIN, DETECT_MAX);
  ...
  //clustering
  CVC_CLUSTER(pcl_roi, cloud_clusters, DELTA_A, DELTA_R, DELTA_P);
  ...
  //std::cout << "cvc segmentation took " << elapsed_time.count() << " milliseconds" << std::endl;
